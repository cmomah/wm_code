...
include ('includesnew/topcontainer_shopping.php');
		...
		if(isset($_SESSION['designerpass']))
		$designerpass = $_SESSION['designerpass'];
		else
		$designerpass      = "";

		if(isset($_SESSION['designeremail']))
		$designeremail = $_SESSION['designeremail'];
		else
		$designeremail      = "";
...
function check_login($dbc, $testquestionanswer = '') {
				...
				if (empty($errors)) { // If everything's OK.
					...
					if ($testquestionanswer == $tqa) {  // if testquestionanswer retrieved from database matches that entered by designer
						...
						$rs=mysql_query("SELECT designeremail, designerpass FROM designers WHERE designeremail = '$designeremail'");

						while ($row = mysql_fetch_array($rs)) {
							$designeremail = $row['designeremail'];
							$designerpass = $row['designerpass'];
						} 
						
							$to=$designeremail;
							$subject = "Information from Waxmode.com";
							$headers = "From: noreply@waxmode.com";
							$body = "Your password to your waxmode.com designer account is: ".$designerpass." \n ";
	...
	if ($check) { // OK!
		...
		// Assign $data to $errors for dessigninhelp2.php:
		$errors = $data;
				...
				<tr>
					...
					<span id="orangetext">Answer the test question displayed </span> <br><br>
						...
						<table border="0" cellspacing="0" cellpadding="0" width="100%">
							...
							<tr>
								<td>
								<form action="dessigninhelp2.php" method="post">
								</td>
							</tr>
							<tr height="40">
